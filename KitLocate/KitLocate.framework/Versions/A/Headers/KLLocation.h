...
#define KL_SP_INT_PARKING_RADIUS                    @"parkingRadius"
...
/*! Add a geofence to KitLocate's database.
 * \param geofenceToAdd The geofence object to add. you should init and set the geofence before calling addGeofence.
 * \param delegate custom class that implements KitLocateDelegate protocol and will accept the callbacks didFailAddGeofence or didSuccessAddGeofence
 */
...
/*! Ask the user to allow location services (if never asked since installation)
 */
+(void)requestForLocationAuthorization;
